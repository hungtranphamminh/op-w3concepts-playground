...
export default function DocuSignV2() {
  ...
  const handleSigChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      console.log("sig update: ", e.target.files[0]);
      const file = e.target.files[0];
      ...
      reader.onloadend = () => {
        // The result attribute contains the data as a Base64 encoded string
        const base64String = reader.result;
        console.log("sig Base64 String: ", base64String);
        SetSigContent(base64String);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  const SetSigContent = (content: string | ArrayBuffer | null) => {
    setDoc((currentDoc) => {
      let tempDoc = { ...currentDoc };
      tempDoc.owner.signaturePDF.content = content;
      return tempDoc;
    });
  };
  ...
  const HandleSubmit = async () => {
    ...
    docToSign.owner.signatureSolana = ownerSignature;

    console.log("full create payload is: ", docToSign);
    await fetch("http://localhost:3000/docs/pair-document-wsig/create", {
      method: "POST",
      headers: {
